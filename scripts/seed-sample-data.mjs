// One-off/dev utility: generates 14 days of realistic sample POS data
// (orders, order items, inventory movements) directly against the live
// database, using the service role key. This is the same generator as
// src/lib/seed.ts::generateDemoData() + the same write path as
// src/lib/actions/settings.ts::seedDemoData() - duplicated here (rather than
// imported) because that pair depends on Next.js request context
// (next/headers, the "@/" path aliases) that plain `node` can't resolve.
// The in-app "Load sample data" button on the Settings page does the same
// thing once you're logged in - use this when you want it done from the
// command line instead (e.g. resetting demo data before a class demo).
//
// Usage:
...
// ---- generation (mirrors src/lib/seed.ts::generateDemoData) ----
...
function generateDemoData(products) {
  ...
  const stock = {};
  const lastUpdatedAt = {};
  for (const p of products) stock[p.id] = randInt(40, 70);
  ...
  const inventoryMap = {};
  for (const p of products) {
    inventoryMap[p.id] = { productId: p.id, stock: stock[p.id], alertAt: 10, updatedAt: lastUpdatedAt[p.id] ?? nowIso };
  }

  return { orders, inventoryMap, movements };
}

// ---- write path (mirrors src/lib/actions/settings.ts::seedDemoData) ----
...
const { orders, inventoryMap, movements } = generateDemoData(products);

console.log(`Generated ${orders.length} orders and ${movements.length} inventory movements. Writing...`);

await supabase.from("inventory_movements").delete().not("id", "is", null);
await supabase.from("orders").delete().not("id", "is", null); // cascades order_items
